    ...
    def get_table(self, fbuf, offset):
        ...
        for record_count in range(0, table_meta[2]):
            record_offset = struct.unpack('>I', fbuf[
                                                RECORD_OFFSET_BASE + (ATOM_SIZE * record_count):RECORD_OFFSET_BASE + (
                                                    ATOM_SIZE * record_count) + ATOM_SIZE])[0]
...
def main():
    ...
    for symmetrickey_record in symmetrickey_list:
        keyblob, ciphertext, iv, return_value = keychain.get_keyblob_record(fbuf, table_list[symmetrickey_offset],
                                                                            symmetrickey_record)
        if return_value == 0:
            passwd = keychain.decrypted_keyblob(ciphertext, iv, dbkey)
